...
public final class ShipCollisionManager {
    ...
    private boolean collidesAt(ShipModel ship, Location position) {
        ...
        for (ShipBlock block : ship.blocks()) {
            double localX = block.x() + 0.5, localZ = block.z() + 0.5;
    ...
    private boolean overlaps(Block block, double cx, double cy, double cz, double half) {
        return cx + half > block.getX() + EPSILON && cx - half < block.getX() + 1.0 - EPSILON
                && cy + 0.5 > block.getY() + EPSILON && cy - 0.5 < block.getY() + 1.0 - EPSILON
                && cz + half > block.getZ() + EPSILON && cz - half < block.getZ() + 1.0 - EPSILON;
    }
    ...
    private boolean insideWorldHeight(ShipModel ship, Location position) {
        ...
        return position.getY() + minY >= world.getMinHeight() && position.getY() + maxY + 1.0 <= world.getMaxHeight();
    }

    private Location interpolate(Location from, Location to, double t) {
        return new Location(from.getWorld(), from.getX() + (to.getX() - from.getX()) * t,
                from.getY() + (to.getY() - from.getY()) * t, from.getZ() + (to.getZ() - from.getZ()) * t,
