...
public final class ShipProjectileDamageManager {
    ...
    public void tick() {
        ...
        for (World world : Bukkit.getWorlds()) {
            for (Entity entity : world.getEntitiesByClass(Projectile.class)) {
                ...
                if (changed) { previousPositions.remove(entity.getUniqueId()); entity.remove(); }
            }
        }
    }

    private boolean applyExplosion(Impact impact, ProjectileType type, org.bukkit.entity.Player source) {
        ShipModel ship = impact.ship(); Location center = impact.location(); ShipRuntimeState runtime = registry.runtime(ship.id()); Location origin = registry.position(ship);
        List<BlockHit> hits = new ArrayList<>();
        for (ShipBlock block : ship.blocks()) {
            Vector local = new Vector(block.x() + 0.5, block.y() + 0.5, block.z() + 0.5);
            double distance = origin.clone().add(forwardTransform(local, ship, runtime)).distance(center);
            if (distance <= type.radius()) hits.add(new BlockHit(block, distance));
        }
        hits.sort(Comparator.comparingDouble(BlockHit::distance));
        boolean changed = false; int affected = 0;
        for (BlockHit hit : hits) {
            double factor = Math.max(0.15, 1.0 - hit.distance() / type.radius());
            if (damage.damageBlock(ship, hit.block(), center, Math.max(0.5, type.damage() * factor), source)) {
                changed = true; if (++affected >= type.maxBlocks()) break;
            }
        }
        return changed;
    }
    ...
    private ProjectileType projectileType(Entity entity) {
        if (cannons != null && cannons.isCannonball(entity)) return new ProjectileType(cannons.damage(), cannons.explosionRadius(), cannons.maxAffectedBlocks(), true);
    ...
    private Impact findImpact(Location start, Location end, World world) {
        ...
        for (ShipModel ship : registry.all()) {
            ...
            Location origin = registry.position(ship); if (origin.getWorld() == null || !origin.getWorld().equals(world)) continue;
            ...
            for (ShipBlock block : ship.blocks()) {
                ...
                double t = segmentAabb(startLocal, endLocal, center.getX() - 0.5 - HIT_EPSILON, center.getX() + 0.5 + HIT_EPSILON, center.getY() - 0.5 - HIT_EPSILON, center.getY() + 0.5 + HIT_EPSILON, center.getZ() - 0.5 - HIT_EPSILON, center.getZ() + 0.5 + HIT_EPSILON);
                if (t >= 0.0 && (best == null || t < best.t())) best = new Impact(ship, block, lerp(start, end, t), t);
            }
        }
        return best;
    }
    private boolean broadPhase(Vector start, Vector end, ShipModel ship) {
        ...
        for (ShipBlock block : ship.blocks()) { minX = Math.min(minX, block.x()); maxX = Math.max(maxX, block.x()); minY = Math.min(minY, block.y()); maxY = Math.max(maxY, block.y()); minZ = Math.min(minZ, block.z()); maxZ = Math.max(maxZ, block.z()); }
        return minX != Integer.MAX_VALUE && segmentAabb(start, end, minX - 1.0, maxX + 2.0, minY - 1.0, maxY + 2.0, minZ - 1.0, maxZ + 2.0) >= 0.0;
    }
    private Vector inverseTransform(Vector vector, ShipModel ship) {
        ShipRuntimeState runtime = registry.runtime(ship.id()); float yaw = registry.position(ship).getYaw(); float pitch = runtime == null ? ship.pitch() : runtime.pitch(); float roll = runtime == null ? 0.0f : runtime.roll();
        Vector v = rotateY(vector, -Math.toRadians(yaw - ship.origin().getYaw())); v = rotateX(v, -Math.toRadians(pitch)); return rotateZ(v, -Math.toRadians(roll));
    }
    private Vector forwardTransform(Vector local, ShipModel ship, ShipRuntimeState runtime) {
        float yaw = registry.position(ship).getYaw(); float pitch = runtime == null ? ship.pitch() : runtime.pitch(); float roll = runtime == null ? 0.0f : runtime.roll();
        Vector v = rotateZ(local, Math.toRadians(roll)); v = rotateX(v, Math.toRadians(pitch)); return rotateY(v, Math.toRadians(yaw - ship.origin().getYaw()));
    }
    ...
    private double segmentAabb(Vector a, Vector b, double minX, double maxX, double minY, double maxY, double minZ, double maxZ) {
        double tMin = 0.0, tMax = 1.0; double[] origin = {a.getX(), a.getY(), a.getZ()}, delta = {b.getX() - a.getX(), b.getY() - a.getY(), b.getZ() - a.getZ()}, min = {minX, minY, minZ}, max = {maxX, maxY, maxZ};
        for (int i = 0; i < 3; i++) { if (Math.abs(delta[i]) < 1.0E-9) { if (origin[i] < min[i] || origin[i] > max[i]) return -1.0; continue; } double inv = 1.0 / delta[i]; double t1 = (min[i] - origin[i]) * inv, t2 = (max[i] - origin[i]) * inv; if (t1 > t2) { double tmp = t1; t1 = t2; t2 = tmp; } tMin = Math.max(tMin, t1); tMax = Math.min(tMax, t2); if (tMin > tMax) return -1.0; }
        return tMin;
    }
    private Location lerp(Location a, Location b, double t) { return a.clone().add(b.toVector().subtract(a.toVector()).multiply(t)); }
    private void cleanupMissingProjectiles() { Iterator<Map.Entry<UUID, Location>> it = previousPositions.entrySet().iterator(); while (it.hasNext()) if (Bukkit.getEntity(it.next().getKey()) == null) it.remove(); }
